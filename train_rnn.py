...
INPUT_SIZE    = len(chars)       # 2 bits per timestep
RNN_HIDDEN    = 20
OUTPUT_SIZE   = len(chars)       # 1 bit per timestep
TINY          = 1e-6    # to avoid NaNs in logs
LEARNING_RATE = 0.01

USE_LSTM = True

inputs  = tf.placeholder(tf.int32, (None, None, INPUT_SIZE))  # (time, batch, in)
outputs = tf.placeholder(tf.int32, (None, None, OUTPUT_SIZE)) # (time, batch, out)
...
NUM_BITS = 10
ITERATIONS_PER_EPOCH = len(data)-NUM_BITS-1
BATCH_SIZE = 16
...
for epoch in range(1000):
    ...
    for i in range(ITERATIONS_PER_EPOCH):
        ...
        x_char = [char2num[ch] for ch in data[i:i+NUM_BITS]]
        y_char = [char2num[ch] for ch in data[i+1:i + NUM_BITS+1]]
        #print(tf.one_hot(x_char,INPUT_SIZE))
        x = np.zeros((x_char.size, x_char.max() + 1))
        x = tf.reshape(tf.one_hot(x_char,INPUT_SIZE),(NUM_BITS,-1,INPUT_SIZE)).eval(session=session)
        y = tf.reshape(tf.one_hot(y_char, OUTPUT_SIZE), (NUM_BITS,-1,OUTPUT_SIZE)).eval(session=session)
